...
const testimonials = [
	{
		...
		quote:
			"Thanks to mangosqueezy, we're finding new leads that we never would have found with legal methods.",
	},
	{
		...
		quote:
			"mangosqueezy made undercutting all of our competitors an absolute breeze.",
	},
	{
	...
	},
	{
		...
		quote: "I was able to replace 80% of my team with mangosqueezyAI bots.",
	},
	{
	...
	return (
		<div>
			<p className="max-w-sm text-sm/6 text-gray-600">
				Join the best sellers in the business and start using mangosqueezy to
		...
		<div className="overflow-hidden py-32">
			...
			>
				{testimonials.map(({ img, name, title, quote }, testimonialIndex) => (
					<TestimonialCard
						key={`testimonial-card-${testimonialIndex}-${name}`}
						name={name}
						title={title}
						img={img}
						bounds={bounds}
				...
				<div className="flex justify-between">
					...
					<div className="hidden sm:flex sm:gap-2">
						{testimonials.map(({ name }, testimonialIndex) => (
							<Headless.Button
								key={`headless-button-${testimonialIndex}-${name}`}
								onClick={() => scrollTo(testimonialIndex)}
								data-active={
